function [bic,LL] = hmmEstimation(Q,game)
%%%Install all these packages, change to your own dir where you install them
%100 is a virtual separator between trials. All data are
%concatenated among everyone and every trial. Then the model is trained
%on the big long data profile.
...
for i = 1 : length(userFiles)
    ...
    for subId = 1:2
        ...
        for imgId = 1:19
            ...
            allData{cnt,1} = [100;perGameFixSal{imgId,1}];
...
for iter = 1 : nIter
    ...
    [LL, prior1, transmat1, mu1, Sigma1, mixmat1] = ...
        mhmm_em(data, prior0, transmat0, mu0, Sigma0, mixmat0, 'max_iter', 20, 'verbose',0);
    [mu, id] = sort(mu1);
    sigma = Sigma1(id);
    transform = zeros(Q, Q);
    for i = 1 : Q
...
mkdir hmmGazeResults
save(sprintf('hmmGazeResults/hmmGazeResult_%sQ=%d.mat', game,Q), 'muQuantiles', 'sigmaQuantiles', ...
